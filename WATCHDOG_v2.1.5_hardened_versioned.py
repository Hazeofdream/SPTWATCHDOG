...
class WatchdogSuiteWindow(QMainWindow):
    ...
    def _build_dashboard_page(self):
        page, layout = self._page_shell("Dashboard", "Monitor status, restart timing, quick controls, and a concise operational summary.")
    ...
    def _build_tray(self):
        ...
        self.tray_icon.setToolTip("Server Panel")
        menu = QMenu(self)
    ...
    def _restart_both_impl(self):
        if not self.restart_lock.acquire(blocking=False):
            self.log("Restart already in progress. Skipping duplicate request.")
            return
        self.restart_in_progress = True
        self._next_headless_launch_id()
        try:
            self.log("Rrestarting both processes...")
            self.send_discord_webhook_async(f"Scheduled restart beginning now.")
            ...
            self.send_discord_webhook_async(f"Scheduled restart completed. Next scheduled restart: {discord_time}")
        finally:
    ...
    def stop_monitor(self):
        ...
        if not self.discord_last_monitor_stop_notice:
            ...
            self.send_discord_webhook_async(f"Monitor stopping.")
        threading.Thread(target=self._stop_both_impl, daemon=True).start()
        self.update_status_ui()
    ...
    def monitor_loop(self):
                    ...
                    if self.config.auto_restart_on_crash:
                        if self.server_proc.process is not None and not self.server_proc.is_running() and not self.server_proc.stop_requested:
                            if self.server_proc.allow_auto_restart():
                                ...
                                self.send_discord_webhook_async(f"Server stopped unexpectedly. Auto-restart triggered.")
                                self._start_server_with_minimize()
                        if self.headless_proc.process is not None and not self.headless_proc.is_running() and not self.headless_proc.stop_requested:
                            if self.headless_proc.allow_auto_restart():
                                ...
                                self.send_discord_webhook_async(f"Headless stopped unexpectedly. Auto-restart triggered.")
                                threading.Thread(target=self._start_headless_with_delay, daemon=True).start()
        ...
        if not self.discord_last_monitor_stop_notice:
            self.discord_last_monitor_stop_notice = True
            self.send_discord_webhook_async(f"Monitor stopped.")
    ...
    def test_webhook(self):
        ...
        if not self.config.discord_webhook_url.strip():
            ...
            return
        self.send_discord_webhook_async(f"Test notification successful. Current time: {datetime.now().strftime('%m/%d/%y %I:%M:%S %p')}", force=True)
...
def install_global_exception_logging():
    ...
    def _sys_excepthook(exc_type, exc_value, exc_traceback):
        ...
        append_log_line(LOG_FILE, f"{datetime.now().strftime('%m/%d/%y %I:%M:%S %p')} [fatal] Unhandled exception\n{formatted}")
        try:
    ...
    if hasattr(threading, "excepthook"):
        ...
        def _threading_excepthook(args):
            ...
            append_log_line(LOG_FILE, f"{datetime.now().strftime('%m/%d/%y %I:%M:%S %p')} [thread-fatal] Unhandled thread exception\n{formatted}")
            try:
...
def main():
    ...
    app.setApplicationName("Server Dashboard")
